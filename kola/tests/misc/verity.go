...
func VerityVerify(c cluster.TestCluster) {
	...
	if hashdev != usrdev {
		// new ACL (dedicated hash partition, azure-container-linux#28+):
		// hash tree starts at the beginning of its own partition.
		c.MustSSH(m, fmt.Sprintf("sudo veritysetup verify --verbose %s %s %s", usrdev, hashdev, hash))
		return
	}

	// old ACL: hash tree appended inline after the btrfs filesystem within
	// usrdev. ACL doesn't ship Seismograph (no e2size), so use btrfs's own
	// superblock to find where the filesystem data ends -- that is exactly
	// where the verity hash tree/superblock begins.
